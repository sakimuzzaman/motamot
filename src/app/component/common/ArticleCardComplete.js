...
const ArticleCardComplete = ({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (
        ...
        setResolvedLinkText("বিস্তারিত দেখুন");
        setResolvedLinkHref("/pollDetails");
      }
    }
  }, []);
  ...
  const handleLinkClick = () => {
    if (pollId) {
      localStorage.setItem("completepollId", pollId);
      if (
        currentPath === "/poll" ||
        currentPath === "/" ||
        currentPath === "/singlePoll"
...
export default ArticleCardComplete;
